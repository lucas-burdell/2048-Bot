...
(function(){
  ...
  var getGridValue = function(grid, x, y){
  ...
  var getHeuristic = function(grid) {
    ...
    for (var y = 0; y < 4; y++){
      for (var x = 0; x < 4; x++){
        ...
        if (x + 1 < 4) {
          var rightValue = getGridValue(grid, x + 1, y);
          if (rightValue === value){
            output += value;
          }
        }
        if (y + 1 < 4) {
          var downValue = getGridValue(grid, x, y + 1);
          if (downValue === value) {
            output += value;
          }
        }
    ...
    while (queuesAreEmpty(queues) === false){
      for (var queueNum = 0; queueNum < queues.length; queueNum++) {
        ...
        for (var direction = 0; direction < 4; direction++){
          sums[queueNum] += getHeuristic(nextBoard) * Math.pow(AI_DISCOUNT, currentDepth);
  ...
  Smoothness.prototype.chooseNextMove = function(grid, previousGrid, lastMoveWorked){
